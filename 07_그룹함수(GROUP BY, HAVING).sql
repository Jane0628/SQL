...
/*
    ...
    (TO_CHAR() 함수를 사용해서 연도만 반환합니다. 그리고 그것을 그룹화합니다.)
*/
SELECT
...
SELECT
    NVL(department_id, 0) AS 부서,
    TO_CHAR(TRUNC(AVG(salary + salary*commission_pct), 2), '$9,999,999.99') AS 평균월급,
    TO_CHAR(SUM(salary + salary*commission_pct), '$9,999,999') AS 월급합계,
    COUNT(salary) AS 직원수
FROM employees
