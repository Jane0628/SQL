...
-- 문제 2.
-- DEPTS 테이블의 데이터를 수정합니다.
-- 1. department_name이 IT Support인 데이터의 department_name을 IT bank로 변경
...
-- 3. department_name이 IT Helpdesk인 데이터의 부서명을 IT Help로, 매니저 아이디를 303으로, 지역 아이디를 1800으로 변경하세요.
UPDATE depts SET department_name = 'IT Help', manager_id = 303, location_id = 1800
WHERE department_name = 'IT Helpdesk';

-- 4. 회계부, 재정, 인사, 영업의 매니저 아이디를 301로 한 번에 변경하세요.
UPDATE depts SET manager_id = 301
...
-- 문제 4.
-- 1. Depts 사본 테이블에서 department_id가 200보다 큰 데이터를 삭제하세요.
DELETE FROM depts WHERE department_id > 200;
SELECT * FROM depts;

-- 2. Depts 사본 테이블의 manager_id가 null이 아닌 데이터의 manager_id를 전부 100으로 변경하세요.
UPDATE depts SET manager_id = 100
WHERE manager_id IS NOT NULL;

-- 3. Depts 테이블은 타겟 테이블입니다.
-- 4. Departments 테이블은 매번 수정이 일어나는 테이블이라고 가정하고 Depts와 비교하여 일치하는 경우 Depts의 부서명, 매니저ID, 지역ID를 업데이트하고 새로 유입된 데이터는 그대로 추가해주는 merge문을 작성하세요.
MERGE INTO depts d
    USING (SELECT * FROM departments) dpt
    ON (d.department_id = dpt.department_id)
WHEN MATCHED THEN
    UPDATE SET
        d.department_name = dpt.department_name,
        d.manager_id = dpt.manager_id,
        d.location_id = dpt.location_id
WHEN NOT MATCHED THEN
    INSERT VALUES
        (dpt.department_id, dpt.department_name,
         dpt.manager_id, dpt.location_id);
...
-- 3. jobs_it은 타겟 테이블입니다.
-- 4. jobs 테이블은 매번 수정이 일어나는 테이블이라고 가정하고 jobs_it과 비교하여 min_salary 컬럼이 0보다 큰 경우 기존의 데이터는 min_salary, max_salary를 업데이트하고 새로 유입된 데이터는 그대로 추가해주는 merge문을 작성하세요.
MERGE INTO jobs_it j
    USING (SELECT * FROM jobs WHERE min_salary > 0) jbs
    ON (j.job_id = jbs.job_id) 
WHEN MATCHED THEN
    UPDATE SET
        j.min_salary = jbs.min_salary,
        j.max_salary = jbs.max_salary
WHEN NOT MATCHED THEN
    INSERT VALUES
        (jbs.job_id, jbs.job_title,
         jbs.min_salary, jbs.max_salary);
