...
CREATE SEQUENCE dept3_seq
    ...
    MINVALUE 1 -- 최소값 (기본값은 증가 1, 감소일 때 -1028)
    NOCACHE -- 캐시메모리 사용 여부 (CACHE)
    NOCYCLE; -- 순환 여부 (NOCYCLE이 기본, 순환시키려면 CYCLE) PRIMARY KEY는 NOCYCLE이 원칙
...
SELECT dept3_seq.CURRVAL FROM dual;
ALTER SEQUENCE dept3_seq INCREMENT BY -28;
...
-- 인덱스 이름 변경
ALTER INDEX SYS_C007050
RENAME TO tbl_board_idx;

SELECT *
FROM (SELECT /*+ INDEX_DESC(tbl_board tbl_board_idx)*/
...
-- 지정된 인덱스를 강제로 쓰게끔 지정.
-- INDEX ASC, DESC를 추가해서 내림차, 오름차순으로 쓰게끔 지정 가능.
