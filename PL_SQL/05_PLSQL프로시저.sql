...
EXEC my_new_job_proc('JOB4', 'test job4', 15000, 20000);
...
IS
    v_cnt NUMBER := 0;
    v_result VARCHAR2(100) := '값이 없어서 INSERT 처리되었습니다.';
BEGIN
    SELECT
        COUNT(*)
    INTO v_cnt
...
DECLARE
    str VARCHAR2(100);
BEGIN
    my_new_job_proc('JOB2', 'test_job2', 2000, 8000, str);
...
CREATE OR REPLACE PROCEDURE my_parameter_test_proc
    (p_var1 IN VARCHAR2,     -- 반환 불가. 받는 용도로만 활용
     p_var2 OUT VARCHAR2,    -- 프로시저가 끝나기 전까지 값의 할당이 안됨. 끝나야만 OUT 가능.
...
BEGIN
    ...
    IF v_cnt = 0 THEN -- 없다면 INSERT
        DBMS_OUTPUT.put_line(p_job_id || '(은)는 테이블에 존재하지 않습니다.');
        RETURN; -- 프로시저 강제 종료.
    END IF;
...
/*
    OTHERS 자리에 예외의 타입을 작성해줍니다.
    OTHERS는 EXCEPTION이라고 생각하면 된다. 예외 전부
    또한 다양한 형태로 저장할 수 있구 
