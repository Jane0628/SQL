...
/*
    ...
    - 일반적인 프로그래밍과는 차이가 있지만, 오라클 내부에서 적절한 처리를 위해 적용해줄 수 있는 절차지향적 코드 작성 방식입니다.
...
-- 연산자
-- 일반 SQL문의 모든 연산자의 사용이 가능하고, **는 제곱을 의미합니다.
DECLARE
    A NUMBER := 2**2*3**2;
BEGIN
...
/*
    - DML문
    DDL문은 사용이 불가능하고, 일반적으로 SQL문의 SELECT 등을 사용하는데, 특이한 점은 SELECT절 아래에 INTO절을 사용해서 변수에 할당할 수 있습니다.
*/

-- 해당 테이블과 같은 타입의 컬럼 변수를 선언하려면
-- 테이블명.컬럼명%TYPE을 사용함으로써 타입을 일일이 확인해야 하는 번거로움을 방지할 수 있다.
DECLARE
