...
-- replace()
SELECT
    REPLACE('My dream is a president :)', 'president', 'programmer') AS test
FROM dual;

SELECT
    REPLACE('My dream is a president :)', ' ', '') AS test
FROM dual;

SELECT
    REPLACE(REPLACE('My dream is a president :)', 'president', 'programmer'), ' ', '') AS test
FROM dual;
...
SELECT
    CONCAT(first_name, LPAD(last_name, LENGTH(last_name)+1, ' ')) AS 이름,
