...
-- auth_id 컬럼을 그냥 쓰면 모호하다고 뜹니다.
-- 그 이유는 양쪽 텡블에 모두 존재하기 때문입니다.
-- 이럴 때는, 컬럼에 테이블 이름을 붙이던지, 별칭을 쓰셔서 확실하게 지목을 해주세요.
SELECT
...
-- CROSS JOIN은 JOIN 조건을 설정하지 않기 때문에 단순히 모든 컬럼에 대해 JOIN을 진행합니다.
-- 실제로는 거의 사용하지 않습니다.
SELECT * FROM info
CROSS JOIN auth
ORDER BY id ASC;
...
SELECT
    ...
    a.job
FROM auth a
LEFT JOIN info i ON a.auth_id = i.auth_id
